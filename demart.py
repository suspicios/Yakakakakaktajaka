...
Author: Claude  
Version: 1.3.3 - AutoADV Bot Flow Fixed + Dummy Payment Fixed + Markdown Fixed
"""
...
"""
    ...
    async def handle_payment_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if callback_data == "buttons_yes":
            ...
            # FIX: Skip directly to image selection step
            state['step'] = 8
    ...
    async def initiate_payment(self, update: Update, context: ContextTypes.DEFAULT_TYPE, state: Dict):
        """Initiate payment process - FIXED VERSION"""
        user_id = update.effective_user.id
        username = update.effective_user.username or "Unknown"
        
        if state['product'] == 'ad':
            amount = 188.0
            product_name = "Advertisement"
        elif state['product'] == 'vip':
            amount = 300.0
...
1️⃣ Send exactly *{amount} USDT* to:
   `{YOUR_USDT_ADDRESS}`

2️⃣ Use TRC20 network only

3️⃣ After sending, click *Verify Payment* below

4️⃣ Your order will activate automatically
...
"""
        ...
        keyboard = [
            [InlineKeyboardButton("✅ Verify Payment", callback_data=f"verify_{payment_id}")],
            [InlineKeyboardButton("🔄 Check Transaction", url=f"https://tronscan.org/#/address/{YOUR_USDT_ADDRESS}")],
            [InlineKeyboardButton("❌ Cancel Order", callback_data=f"cancel_{payment_id}")]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        
        if hasattr(update, 'message'):
            await update.message.reply_text(payment_text, parse_mode=ParseMode.MARKDOWN, reply_markup=reply_markup)
        else:
            await update.callback_query.edit_message_text(payment_text, parse_mode=ParseMode.MARKDOWN, reply_markup=reply_markup)
    
    @error_handler
    async def process_dummy_payment_auto(self, update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int, 
                                       product: str, amount: float, data: Dict, payment_id: str):
        """Automatically process dummy payment - FIXED VERSION"""
        try:
            # Record purchase
            async with db_manager.get_connection() as db:
                await db.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """, (user_id, "dummy_user", product, amount, f"DUMMY_{payment_id}", "completed", 
            ...
            if product == 'ad':
                await self.finalize_purchase_with_payment(update, context, user_id, product, amount, data, f"DUMMY_{payment_id}")
            elif product == 'vip':
                await self.finalize_purchase_with_payment(update, context, user_id, product, amount, data, f"DUMMY_{payment_id}")
...
✅ Your {product.upper()} purchase has been completed!

💰 Amount: {amount} USDT
📦 Product: {product.title()}
🆔 Transaction: DUMMY_{payment_id}
...
"""
            ...
            logger.error(f"Error in dummy payment: {e}")
            error_msg = "❌ Error processing payment. Please contact support."
            if hasattr(update, 'message'):
    ...
    async def verify_payment_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        async with db_manager.get_connection() as db:
            cursor = await db.execute("""
                SELECT product, amount, data FROM pending_payments 
        ...
        product, amount, data_json = pending
        data = json.loads(data_json)
        
        if PAYMENT_MODE == "dummy":
            # Auto-verify dummy payment
            await query.edit_message_text("🔄 Verifying dummy payment...")
            await asyncio.sleep(2)
            
            await self.finalize_purchase_with_payment(update, context, user_id, product, amount, data, f"VERIFIED_{payment_id}")
            return
        
        # Real payment verification would go here
        # For now, simulate verification
        await query.edit_message_text("🔄 Checking transaction on blockchain...")
        await asyncio.sleep(3)
        
        # Simulate successful verification
        verified = True  # In real implementation, check blockchain
        
        if verified:
            await self.finalize_purchase_with_payment(update, context, user_id, product, amount, data, f"VERIFIED_{payment_id}")
        else:
            await query.edit_message_text("❌ Payment not found! Please wait for blockchain confirmation.")
    
    @error_handler
    async def finalize_purchase_with_payment(self, update: Update, context: ContextTypes.DEFAULT_TYPE, 
                                           user_id: int, product: str, amount: float, data: Dict, tx_hash: str):
        """Finalize purchase after payment verification - FIXED VERSION"""
        try:
            username = update.effective_user.username or "Unknown"
            
            if product == 'ad':
                await self.process_ad_purchase(update, context, user_id, username, data, tx_hash)
            elif product == 'vip':
                await self.process_vip_purchase(update, context, user_id, username, data, tx_hash)
            ...
            await ConfigManager.update_statistics(f"purchase_{product}")
            
        except Exception as e:
            logger.error(f"Error finalizing purchase: {e}")
            await ConfigManager.log_error("autoadv_bot", "finalize_purchase", str(e), traceback.format_exc())
            
            error_msg = "❌ Error completing purchase. Please contact support with your transaction hash."
            if hasattr(update, 'message'):
                await update.message.reply_text(error_msg)
            else:
                await update.callback_query.edit_message_text(error_msg)
    ...
    @error_handler
    async def process_ad_purchase(self, update: Update, context: ContextTypes.DEFAULT_TYPE, 
                                user_id: int, username: str, data: Dict, tx_hash: str):
        """Process advertisement purchase - FIXED MARKDOWN PARSING"""
        try:
            # Calculate expiration (10 days from now)
            expires_at = datetime.now() + timedelta(days=10)
            
            # Prepare button data
            button_data = []
            if data.get('has_buttons') and data.get('button_text') and data.get('button_url'):
                button_data = [{'text': data['button_text'], 'url': data['button_url']}]
            ...
            async with db_manager.get_connection() as db:
                await db.execute("""
                    ...
                    (user_id, username, heading, type, description, contact, created_at, expires_at, 
                     status, post_count, has_buttons, button_data, has_image, image_url)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'active', 0, ?, ?, ?, ?)
                """, (user_id, username, data['heading'], data['type'], data['description'], data['contact'],
                      datetime.now().isoformat(), expires_at.isoformat(),
                      int(data.get('has_buttons', False)), json.dumps(button_data),
                      int(data.get('has_image', False)), data.get('image_url', '')))
                await db.commit()
            
            # FIXED: Escape Markdown special characters in user-provided data
            ...
            # Prepare success message with escaped user data
            escaped_heading = escape_markdown(data['heading'])
            escaped_type = escape_markdown(data['type'])
...
📋 *Ad Details:*
▫️ Heading: {escaped_heading}
▫️ Type: {escaped_type}
▫️ Contact: {escaped_contact}
▫️ Duration: 10 days
▫️ Status: Active
...
"""
            ...
            logger.info(f"✅ Ad purchase completed for user {user_id}")
            
        except Exception as e:
            logger.error(f"Error in process_ad_purchase: {e}")
            await ConfigManager.log_error("autoadv_bot", "process_ad_purchase", str(e), traceback.format_exc())
            raise
    
    @error_handler
    async def process_vip_purchase(self, update: Update, context: ContextTypes.DEFAULT_TYPE, 
                                 user_id: int, username: str, data: Dict, tx_hash: str):
        """Process VIP purchase - FIXED MARKDOWN PARSING"""
        try:
            # Calculate expiration (60 days from now)
            expires_at = datetime.now() + timedelta(days=60)
            
            # Store in VIP members
            async with db_manager.get_connection() as db:
                await db.execute("""
            ...
            escaped_phone = escape_markdown(data['phone'])
            escaped_email = escape_markdown(data['email'])
...
📋 *VIP Details:*
▫️ Name: {escaped_name}
▫️ Phone: {escaped_phone}
▫️ Email: {escaped_email}
▫️ Duration: 60 days
...
• Automatically verified in main group
• No message length restrictions
• Premium support priority

🌟 *Thank you for joining VIP!*
"""
            ...
            logger.info(f"✅ VIP purchase completed for user {user_id}")
            
        except Exception as e:
            logger.error(f"Error in process_vip_purchase: {e}")
            await ConfigManager.log_error("autoadv_bot", "process_vip_purchase", str(e), traceback.format_exc())
            raise
...
📋 *Scammer Details:*
{state['data']['scammer_details']}

🔍 *Evidence:*
{state['data']['evidence']}

📞 *Reporter Contact:* {state['data']['contact']}

⚠️ *WARNING: Be cautious when dealing with this user!*
"""
            ...
            logger.info(f"✅ Scammer report submitted by user {user_id}")
            
        except Exception as e:
            logger.error(f"Error submitting scammer report: {e}")
            await update.message.reply_text(
...
"""
        ...
        for purchase in purchases:
            ...
            purchases_text += f"{status_emoji} *{product_type.upper()}*\n"
            purchases_text += f"💰 Amount: {amount} USDT\n"
            purchases_text += f"📅 Date: {created}\n"
    ...
    async def help_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Help command"""
        help_text = """
🆘 *AUTO ADV BOT - HELP CENTER* 🆘

🤖 *ABOUT THIS BOT:*
Auto ADV Bot is your automated marketplace for premium Telegram services including advertisements, VIP memberships, and scammer reports.
...
• Contact support for refunds/issues
"""
    ...
    def setup_handlers(self):
        """Setup all handlers for Auto ADV bot"""
        try:
            # Command handlers
            ...
            self.app.add_handler(CallbackQueryHandler(self.handle_payment_callback, pattern="^(buttons_|image_|verify_|cancel_)"))
            
            # Message handler for purchase flow
...
# ============================
# 🚀 MAIN SYSTEM - COMPLETE FIXED VERSION
# ============================
...
class InterlinkBotSystem:
    ...
    async def initialize_system(self):
        ...
        logger.info("🚀 INITIALIZING INTERLINK BOT SYSTEM...")
    ...
    async def run_system(self):
        """Run the complete bot system with enhanced error handling"""
        ...
        try:
            ...
            for bot_name, bot_instance in self.bots.items():
                task = asyncio.create_task(self.run_bot_safely(bot_name, bot_instance))
                bot_tasks.append(task)
                await asyncio.sleep(1)  # Stagger bot startups
            
            # Wait for all bots to complete (they shouldn't unless there's an error)
    ...
    async def run_bot_safely(self, bot_name: str, bot_instance):
        """Run a bot with comprehensive error handling"""
        max_restarts = 5
        restart_delay = 10
        
        for attempt in range(max_restarts):
            try:
                logger.info(f"🚀 Starting {bot_name} bot (Attempt {attempt + 1}/{max_restarts})...")
                await bot_instance.run()
                break
                
            except Exception as e:
                logger.error(f"❌ {bot_name} bot crashed (Attempt {attempt + 1}): {e}")
                await ConfigManager.log_error(bot_name, "bot_crash", str(e), traceback.format_exc())
                
                if attempt < max_restarts - 1:
                    logger.info(f"🔄 Restarting {bot_name} bot in {restart_delay} seconds...")
                    await asyncio.sleep(restart_delay)
                    restart_delay *= 2  # Exponential backoff
                else:
                    logger.error(f"❌ {bot_name} bot failed after {max_restarts} attempts")
    
    async def shutdown_system(self):
        """Safely shutdown the entire system"""
        logger.info("🛑 SHUTTING DOWN INTERLINK BOT SYSTEM...")
        self.is_running = False
        ...
        try:
            ...
            for bot_name, bot_instance in self.bots.items():
                if hasattr(bot_instance, 'safe_shutdown'):
                    ...
                    task = asyncio.create_task(self.force_shutdown_bot(bot_instance))
                    shutdown_tasks.append(task)
    ...
    async def force_shutdown_bot(self, bot_instance):
        """Force shutdown a bot instance"""
        try:
            if hasattr(bot_instance, 'app'):
