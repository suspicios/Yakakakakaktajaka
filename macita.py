...
Author: Claude  
Version: 1.4.3 - Fixed Scammer Report Image Handling + Admin Approval Flow
"""
...
"""
    ...
    async def start_scammer_report(self, query, context, user_id):
        ...
        await query.edit_message_text(
            ...
            "━━━━━━━━━━━━━━━━\n"
            "📝 *Step 1 of 6: Enter Scammer Details*\n\n"
    ...
    async def handle_scammer_flow(self, update: Update, context: ContextTypes.DEFAULT_TYPE, state: Dict, message: str):
        """Handle scammer report flow - FIXED VERSION"""
        user_id = update.effective_user.id
        ...
        if state['step'] == 1:  # Scammer details
            ...
            await update.message.reply_text(
                "✅ *Evidence saved!*\n\n"
                "━━━━━━━━━━━━━━━━\n"
                "📝 *Step 3 of 6: Enter Your Contact*\n\n"
            ...
            await update.message.reply_text(
                "✅ *Contact saved!*\n\n"
                "━━━━━━━━━━━━━━━━\n"
                "📝 *Step 4 of 6: Communication Screenshot*\n\n"
                "Please provide screenshot of communication with scammer:\n"
                "_(Send the image file or provide image URL)_",
                parse_mode=ParseMode.MARKDOWN
            )
        
        elif state['step'] == 4:  # Communication screenshot - WAITING FOR IMAGE
            # This step is handled by the separate image handler
            pass
        
        elif state['step'] == 5:  # USDT Address
    ...
    async def handle_scammer_image_upload(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Handle image uploads for scammer reports - COMPLETELY FIXED VERSION"""
        user_id = update.effective_user.id
        ...
        if state['product'] != 'scammer' or state['step'] != 4:
            return
        ...
        if update.message.photo:
            ...
            # Store file_id in the state data
            state['data']['communication_screenshot'] = file_id
            state['step'] = 5
            
            await update.message.reply_text(
                "✅ *Screenshot received and saved!*\n\n"
                "━━━━━━━━━━━━━━━━\n"
                "📝 *Step 5 of 6: Scammer's USDT Address*\n\n"
                "Please provide the scammer's USDT address (if known):\n"
                "Or type 'unknown' if not available:",
                parse_mode=ParseMode.MARKDOWN
            )
        else:
            await update.message.reply_text(
                "❌ Please send an image file as screenshot evidence.",
                parse_mode=ParseMode.MARKDOWN
            )
...
"""
    ...
    async def process_scammer_report(self, update: Update, context: ContextTypes.DEFAULT_TYPE, data: Dict, user_id: int):
        """Process scammer report - SEND TO ADMIN DM FOR APPROVAL - FIXED VERSION"""
        # Save to database for admin approval
        async with db_manager.get_connection() as db:
            cursor = await db.execute("""
                INSERT INTO scammer_reports 
                (user_id, username, scammer_details, evidence, contact, communication_screenshot, scammer_usdt_address, status, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, 'pending', ?)
            """, (
                user_id,
                ...
                data['evidence'],
                data['contact'],
                data.get('communication_screenshot', ''),
...
{'✅ Screenshot provided' if data.get('communication_screenshot') else '❌ No screenshot'}
"""
        ...
        for admin_id in ADMIN_IDS:
            try:
                ...
                # If there's a screenshot, send it with the report
                if data.get('communication_screenshot'):
                    try:
                        await self.app.bot.send_photo(
                            chat_id=admin_id,
                            photo=data['communication_screenshot'],
                ...
                logger.info(f"✅ Scammer report #{report_id} sent to admin {admin_id} for approval")
                
            except Exception as e:
                logger.error(f"❌ Could not send scam report to admin {admin_id}: {e}")

        success_text = """
✅ *SCAMMER REPORT SUBMITTED FOR APPROVAL!*
...
"""
    ...
    async def handle_scam_approval_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Handle scam report approval/rejection by admin - FIXED VERSION"""
        query = update.callback_query
        await query.answer()
    ...
    async def approve_scam_report(self, query, context, report_id: int, admin_id: int):
        """Approve a scam report and post to scammer channel - FIXED VERSION"""
        # Get report details from database
        async with db_manager.get_connection() as db:
            cursor = await db.execute("""
                SELECT username, scammer_details, evidence, scammer_usdt_address, communication_screenshot
            ...
            reporter_username, scammer_details, evidence, usdt_address, screenshot = report
...
"""
        ...
        # Post to scammer channel - WITH SCREENSHOT IF AVAILABLE
        try:
            if screenshot:
                ...
                try:
                    ...
                    await self.app.bot.send_message(
                        chat_id=SCAMMER_EXPOSED_ID,
                        text=scammer_post + f"\n\n📎 *Screenshot available in admin records*",
                        parse_mode=ParseMode.MARKDOWN
                    )
            else:
                # Send text only
    ...
    async def view_pending_scam_reports(self, query, context):
        ...
        async with db_manager.get_connection() as db:
            cursor = await db.execute("""
                SELECT id, username, scammer_details, created_at 
        ...
        for report in pending_reports:
            report_id, username, details, created_at = report
...
"""
    ...
    def setup_handlers(self):
        ...
        try:
            ...
            # Image handler for scammer reports - FIXED: Now properly handles image uploads
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info("🔧 Version: 1.4.3 - Fixed Scammer Report Image Handling + Admin Approval Flow")
