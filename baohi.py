...
class AutoAdvPaymentBot:
    def __init__(self, token: str):
        ...
        self.ad_purchase_data = {}  # Store temporary ad purchase data
...
"""
    ...
    async def receive_ad_type(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        package_map = {
            "ad_type_basic": {"name": "Basic", "price": 5, "duration": "1 week"},
            "ad_type_premium": {"name": "Premium", "price": 15, "duration": "1 month"},
            "ad_type_enterprise": {"name": "Enterprise", "price": 30, "duration": "3 months"}
        }
        ...
        await query.edit_message_text(
            ...
            "We provide daily analysis and expert insights to maximize your profits.'\n\n"
            "📝 *Tips:*\n"
...
"""
    ...
    async def process_ad_payment(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        if PAYMENT_MODE == "dummy":
            # Dummy mode - process immediately without payment
            await query.edit_message_text(
                "🔍 *PROCESSING PAYMENT* 🔍\n\n"
            ...
            created_at = datetime.now()
            expires_at = created_at + timedelta(weeks=1 if package['name'] == 'Basic' else 4 if package['name'] == 'Premium' else 12)
...
📢 It will be automatically posted every 5-6 minutes in our groups.
...
⚡ *Payment will auto-verify once confirmed on blockchain*
"""
            await query.edit_message_text(
                payment_text,
                parse_mode=ParseMode.MARKDOWN
...
"""
    ...
    def setup_handlers(self):
        ...
        # Ad purchase conversation handler
        ad_purchase_conv = ConversationHandler(
            ...
            states={
                ...
                AD_TYPE: [CallbackQueryHandler(self.receive_ad_type, pattern="^(ad_type_|cancel_ad)")],
                ...
                AD_CONFIRMATION: [CallbackQueryHandler(self.process_ad_payment, pattern="^(confirm_ad_payment|edit_ad_details|cancel_ad_final)")]
            },
