...
"""
                    # Using actual bot usernames and links
                    keyboard = [
...
"""
                    # Using actual links
                    keyboard = [
                        [InlineKeyboardButton("💎 Join VIP", url=VIP_CHANNEL_LINK)],
                        ...
                        [InlineKeyboardButton("⚠️ Report Scammer", url=SCAMMER_EXPOSED_LINK)]
                    ]
        ...
        if vip:
            ...
            if expires > datetime.now():
                ...
                await update.message.reply_text(
                    ...
                    f"🔄 They can renew anytime via {AUTOADV_BOT_USERNAME}!",
                    parse_mode=ParseMode.MARKDOWN
                )
        else:
            await update.message.reply_text(
...
"""
    ...
    async def verify_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if update.effective_user.id not in ADMIN_IDS:
            await update.message.reply_text("⛔ Only Supreme Admins can manually verify users!")
            return
        
        if not context.args:
            await update.message.reply_text("📝 Usage: /groupverify @username")
            return
        
        username = context.args[0].replace("@", "")
        
        async with aiosqlite.connect(DB_NAME) as db:
            await db.execute("UPDATE new_members SET verified=1 WHERE username=?", (username,))
            await db.commit()
        
        await update.message.reply_text(
            f"✅ *USER VERIFIED!* ✅\n\n"
            f"@{username} has been manually verified and granted full access.",
            parse_mode=ParseMode.MARKDOWN
        )
    
    async def exempt_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Exempt user from verification (Admin only)"""
        if update.effective_user.id not in ADMIN_IDS:
            await update.message.reply_text("⛔ Only Supreme Admins can exempt users!")
            return
        
        if not context.args:
            await update.message.reply_text("📝 Usage: /groupexempt @username")
            return
        
        username = context.args[0].replace("@", "")
        
        async with aiosqlite.connect(DB_NAME) as db:
            # Get user ID (simplified - in production use proper resolution)
            cursor = await db.execute("SELECT user_id FROM new_members WHERE username=?", (username,))
            ...
            if user:
                ...
                await update.message.reply_text(
                    f"✅ *USER EXEMPTED!* ✅\n\n"
                    f"@{username} is now exempt from verification.\n\n"
                    f"They will have full access immediately upon joining.",
                    parse_mode=ParseMode.MARKDOWN
                )
            else:
                await update.message.reply_text("❌ User not found in new members!")
    
    async def remove_exempt_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Remove user exemption (Admin only)"""
        if update.effective_user.id not in ADMIN_IDS:
            await update.message.reply_text("⛔ Only Supreme Admins can remove exemptions!")
            return
        
        if not context.args:
            await update.message.reply_text("📝 Usage: /groupremoveexempt @username")
            return
        
        username = context.args[0].replace("@", "")
        
        async with aiosqlite.connect(DB_NAME) as db:
            await db.execute("DELETE FROM exempted_users WHERE username=?", (username,))
            await db.commit()
        
        await update.message.reply_text(
            f"✅ *EXEMPTION REMOVED!* ✅\n\n"
            f"@{username} will now require verification upon joining.",
            parse_mode=ParseMode.MARKDOWN
        )
    ...
    async def new_member_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Handle new members joining - REQUIRE JOINING ALL CHANNELS"""
        for member in update.message.new_chat_members:
            ...
            # Send channel joining requirement message
            keyboard = [
                ...
                [InlineKeyboardButton("📂 All in One Folder", url=ALL_IN_ONE_FOLDER_LINK)],
                [InlineKeyboardButton("✅ I've Joined All", callback_data=f"joined_all_{user_id}")]
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
            
            await update.message.reply_text(
                f"🛡️ *CHANNEL JOINING REQUIRED* 🛡️\n\n"
                f"👤 @{username} has joined the group!\n\n"
                f"📋 *Please join all our channels to get full access:*\n\n"
                f"📱 *Main Group* - Main discussion hub\n"
                f"🏢 *Company Resources* - Business opportunities\n"
                f"💎 *VIP Channel* - Exclusive content\n"
                f"⚠️ *Scammer Exposed* - Safety alerts\n"
                f"📂 *All in One Folder* - Complete collection\n\n"
                f"🔗 Click all buttons above to join\n"
                f"✅ Then click 'I've Joined All' to verify\n\n"
    ...
    async def button_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        data = query.data
        user_id = int(data.split("_")[2])  # joined_all_12345
        
        if data.startswith("joined_all_"):
            # Verify user after they claim to have joined all channels
            async with aiosqlite.connect(DB_NAME) as db:
            ...
            await query.edit_message_text(
                ...
                "Thank you for joining all our channels! You now have full access to the group.\n\n"
                "🌟 *What you get:*\n"
                "✅ Full group access\n"
    ...
    def setup_handlers(self):
        ...
        self.app.add_handler(CommandHandler(f"{self.bot_prefix}exempt", self.exempt_command))
        self.app.add_handler(CommandHandler(f"{self.bot_prefix}removeexempt", self.remove_exempt_command))
...
"""
    ...
    async def process_advertisement_purchase(self, user_id: int, update: Update):
        ...
        await update.message.reply_text(
            ...
            "✅ Runs for 30 days\n\n"
            f"📊 Monitor with {ADV_BOT_USERNAME}\n"
    ...
    async def process_vip_purchase(self, user_id: int, update: Update):
        ...
        success_message += (
            f"🔍 Check your status with {VIP_BOT_USERNAME}\n"
