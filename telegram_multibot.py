...
Author: Claude  
Version: 1.4.4 - Enhanced Multi-Image Support for Scammer Reports + All Features Restored
"""
...
async def init_database():
    ...
    try:
        async with db_manager.get_connection() as db:
            ...
            # Scammer reports table - ENHANCED FOR MULTIPLE IMAGES
            await db.execute("""
                CREATE TABLE IF NOT EXISTS scammer_reports (
                    ...
                    evidence TEXT,
                    contact TEXT,
                    communication_screenshots TEXT DEFAULT '[]',
                    scammer_usdt_address TEXT,
...
# ============================
# 🤖 1. ADVERTISING BOT - COMPLETE WITH ALL FEATURES
# ============================
...
/{self.prefix}_free_ad - Create free ad (Admin only)
"""
    ...
    @error_handler
    async def free_ad_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Create free advertisement (Admin only)"""
        if update.effective_user.id not in ADMIN_IDS:
            await update.message.reply_text("⛔ Only Supreme Admins can create free ads!")
            return
        
        # Store admin state for ad creation
        user_id = update.effective_user.id
        context.user_data['admin_free_ad'] = True
        context.user_data['ad_creation_step'] = 1
        
        await update.message.reply_text(
            "🎯 *ADMIN FREE AD CREATION* 🎯\n\n"
            "You are creating a free advertisement as an admin.\n\n"
            "━━━━━━━━━━━━━━━━\n"
    ...
    @error_handler
    async def handle_admin_ad_creation(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Handle admin free ad creation flow"""
        if update.effective_user.id not in ADMIN_IDS:
            return
            
        if not context.user_data.get('admin_free_ad'):
            return
            
        user_id = update.effective_user.id
        step = context.user_data.get('ad_creation_step', 1)
        ...
        if step == 1:  # Heading
            ...
            context.user_data['ad_heading'] = user_message
            context.user_data['ad_creation_step'] = 2
            ...
            context.user_data['ad_type'] = user_message
            context.user_data['ad_creation_step'] = 3
            ...
            context.user_data['ad_description'] = user_message
            context.user_data['ad_creation_step'] = 4
            ...
            context.user_data['ad_contact'] = user_message
            context.user_data['ad_creation_step'] = 5
            
            keyboard = [
                [InlineKeyboardButton("✅ Yes", callback_data="admin_buttons_yes")],
                [InlineKeyboardButton("❌ No", callback_data="admin_buttons_no")]
            ]
        ...
        elif step == 6:  # Button text
            context.user_data['ad_button_text'] = user_message
            context.user_data['ad_creation_step'] = 7
            ...
            context.user_data['ad_button_url'] = user_message
            context.user_data['ad_creation_step'] = 8
            
            keyboard = [
                [InlineKeyboardButton("✅ Yes", callback_data="admin_image_yes")],
                [InlineKeyboardButton("❌ No", callback_data="admin_image_no")]
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
        ...
        elif step == 8:  # Image URL
            if not user_message.startswith(('http://', 'https://')):
                await update.message.reply_text("❌ Invalid URL! Must start with http:// or https://. Try again:")
                return
            
            context.user_data['ad_image_url'] = user_message
            await self.finalize_admin_ad(update, context)
    
    @error_handler
    async def handle_admin_ad_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Handle admin ad creation callbacks"""
        ...
        if callback_data == "admin_buttons_yes":
            context.user_data['ad_has_buttons'] = True
            context.user_data['ad_creation_step'] = 6
        ...
        elif callback_data == "admin_buttons_no":
            context.user_data['ad_has_buttons'] = False
            context.user_data['ad_creation_step'] = 8
        ...
        elif callback_data == "admin_image_yes":
            context.user_data['ad_has_image'] = True
        ...
        elif callback_data == "admin_image_no":
            context.user_data['ad_has_image'] = False
            await self.finalize_admin_ad(update, context)
    
    @error_handler
    async def finalize_admin_ad(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Finalize and create admin free ad"""
        user_id = update.effective_user.id
        
        # Calculate expiration (30 days for admin ads)
        expires_at = datetime.now() + timedelta(days=30)
        
        # Prepare button data
        button_data = []
        if context.user_data.get('ad_has_buttons'):
            button_data.append({
                'text': context.user_data.get('ad_button_text', 'Contact'),
                'url': context.user_data.get('ad_button_url', '')
            })
        ...
        async with db_manager.get_connection() as db:
            await db.execute("""
                ...
                (user_id, username, heading, type, description, contact, created_at, expires_at, has_buttons, button_data, has_image, image_url)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                user_id,
                ...
                1 if context.user_data.get('ad_has_buttons') else 0,
                json.dumps(button_data),
                1 if context.user_data.get('ad_has_image') else 0,
                context.user_data.get('ad_image_url', '')
            ))
            await db.commit()
        ...
        await update.message.reply_text(
            "🎉 *ADMIN AD CREATED SUCCESSFULLY!* 🎉\n\n"
            "Your advertisement has been added to the queue for FREE!\n\n"
            "📊 *Ad Details:*\n"
            f"• Heading: {context.user_data['ad_heading']}\n"
            f"• Type: {context.user_data['ad_type']}\n"
            f"• Duration: 30 days\n"
            f"• Status: Active\n\n"
            "Your ad will start appearing in the rotation immediately!",
            parse_mode=ParseMode.MARKDOWN
        )
    ...
    async def stats_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            async with db_manager.get_connection() as db:
                ...
                # Total posts made
                cursor = await db.execute("SELECT SUM(post_count) FROM ads_queue")
                total_posts_result = await cursor.fetchone()
                total_posts = total_posts_result[0] if total_posts_result[0] else 0
                
                # Today's posts
                today = datetime.now().date().isoformat()
                cursor = await db.execute("SELECT COUNT(*) FROM statistics WHERE metric_name='ad_posted' AND DATE(recorded_at)=?", (today,))
                today_posts_result = await cursor.fetchone()
                today_posts = today_posts_result[0] if today_posts_result else 0
                
                # Get ad config
                cursor = await db.execute("SELECT is_paused, post_interval FROM ad_config WHERE id=1")
                config_result = await cursor.fetchone()
                is_paused = config_result[0] if config_result else 0
                post_interval = config_result[1] if config_result else 5
            
            status_icon = "⏸️" if is_paused else "▶️"
            status_text = "PAUSED" if is_paused else "ACTIVE"
            
            stats_text = (
                f"📊 *ADVERTISING STATISTICS* 📊\n\n"
                f"🔄 *System Status:* {status_icon} {status_text}\n"
                f"⏱️ *Post Interval:* {post_interval} minutes\n\n"
                f"📈 *Advertisement Stats:*\n"
                f"• Active Ads: {active_ads}\n"
                f"• Total Ads: {total_ads}\n"
                f"• Total Posts: {total_posts}\n"
                f"• Today's Posts: {today_posts}\n\n"
                f"💫 *Next Post:* In {post_interval} minutes"
            )
    ...
    @error_handler
    async def viewqueue_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """View all ads in queue"""
        try:
            async with db_manager.get_connection() as db:
                cursor = await db.execute("""
                    SELECT id, user_id, username, heading, type, created_at, post_count, status
                    FROM ads_queue 
                    ORDER BY created_at DESC
                    LIMIT 20
                """)
            ...
            if not ads:
                await update.message.reply_text("📭 No advertisements in queue.")
                return
            
            queue_text = "📋 *ADVERTISEMENT QUEUE* 📋\n\n"
            
            for ad in ads:
                ad_id, user_id, username, heading, ad_type, created_at, post_count, status = ad
                created_date = datetime.fromisoformat(created_at).strftime("%Y-%m-%d")
                
                status_icon = "🟢" if status == "active" else "🔴"
                
                queue_text += (
                    f"{status_icon} *AD #{ad_id}*\n"
                    f"📝 {heading[:50]}...\n"
                    f"👤 {username} | 🏷️ {ad_type}\n"
                    f"📅 {created_date} | 📊 {post_count} posts\n"
                    f"━━━━━━━━━━━━━━━━\n"
                )
            ...
            logger.error(f"Error in viewqueue_command: {e}")
            await update.message.reply_text("❌ Error retrieving ad queue. Please try again.")
    
    @error_handler
    async def pause_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Pause advertising system (Admin only)"""
        if update.effective_user.id not in ADMIN_IDS:
            await update.message.reply_text("⛔ Only Supreme Admins can pause advertising!")
            return
        ...
        await update.message.reply_text(
            ...
            "All automatic ad posting has been temporarily suspended.\n"
            "Use /adv_resume to resume advertising operations.",
            parse_mode=ParseMode.MARKDOWN
        )
    
    @error_handler
    async def resume_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Resume advertising system (Admin only)"""
        if update.effective_user.id not in ADMIN_IDS:
            await update.message.reply_text("⛔ Only Supreme Admins can resume advertising!")
            return
        ...
        await update.message.reply_text(
            ...
            "Automatic ad posting has been resumed successfully!\n"
            "Ads will now continue their normal rotation schedule.",
            parse_mode=ParseMode.MARKDOWN
        )
    
    @error_handler
    async def removead_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Remove specific advertisement (Admin only)"""
        if update.effective_user.id not in ADMIN_IDS:
            await update.message.reply_text("⛔ Only Supreme Admins can remove ads!")
            return
        
        if not context.args or not context.args[0].isdigit():
            await update.message.reply_text("❌ Usage: /adv_removead <ad_id>")
            return
        ...
        await update.message.reply_text(
            f"🗑️ *ADVERTISEMENT REMOVED* 🗑️\n\n"
            f"Ad ID: {ad_id}\n"
            f"Heading: {ad[0]}\n\n"
            f"The advertisement has been permanently removed from the queue.",
            parse_mode=ParseMode.MARKDOWN
        )
    
    @error_handler
    async def setinterval_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Set advertising post interval (Admin only)"""
        if update.effective_user.id not in ADMIN_IDS:
            await update.message.reply_text("⛔ Only Supreme Admins can set intervals!")
            return
        
        if not context.args or not context.args[0].isdigit():
            await update.message.reply_text("❌ Usage: /adv_setinterval <minutes>")
            return
        ...
        await update.message.reply_text(
            f"⏱️ *POST INTERVAL UPDATED* ⏱️\n\n"
            f"New interval: {interval} minutes\n\n"
            f"Ads will now be posted every {interval} minutes.",
            parse_mode=ParseMode.MARKDOWN
        )
    
    @error_handler
    async def forcead_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Force post next ad immediately (Admin only)"""
        if update.effective_user.id not in ADMIN_IDS:
            await update.message.reply_text("⛔ Only Supreme Admins can force post ads!")
            return
        ...
        try:
            ...
            async with db_manager.get_connection() as db:
                ...
                ad_id, heading, ad_type, description, contact, has_buttons, button_data, has_image, image_url = ad
                ...
                ad_text = (
                    ...
                    f"📞 *Contact:* {contact}\n"
                    f"🏷️ *Type:* {ad_type}\n\n"
                    f"💫 _Posted via InterLink Advertising System_"
                )
                
                # Prepare buttons if available
                reply_markup = None
                if has_buttons and button_data:
                    try:
                ...
                try:
                    ...
                    # Update post count
                    await db.execute("UPDATE ads_queue SET post_count = post_count + 1 WHERE id=?", (ad_id,))
                    await db.commit()
                    ...
                    await update.message.reply_text(
                        f"✅ *AD FORCE-POSTED SUCCESSFULLY!*\n\n"
                        f"📢 Ad: {heading}\n"
                        f"🎯 Type: {ad_type}\n"
                        f"📍 Pinned: ✅\n"
                        f"📊 Post Count Updated",
                        parse_mode=ParseMode.MARKDOWN
                    )
                    
                except Exception as e:
                    logger.error(f"Error posting ad: {e}")
                    await update.message.reply_text("❌ Error posting ad to group!")
                
        except Exception as e:
            logger.error(f"Error in forcead_command: {e}")
            await update.message.reply_text("❌ Error force-posting ad!")
    ...
    async def system_stats_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if update.effective_user.id not in ADMIN_IDS:
            await update.message.reply_text("⛔ Only Supreme Admins can view system stats!")
            return
        
        try:
            stats = await StatisticsManager.get_system_stats()
            analytics = await StatisticsManager.get_revenue_analytics(7)
            
            # Format revenue data
            weekly_revenue = sum(day[1] for day in analytics['daily_revenue']) if analytics['daily_revenue'] else 0
            
            stats_text = (
                "🌐 *INTERLINK SYSTEM STATISTICS* 🌐\n\n"
                ...
                "📈 *TODAY'S ACTIVITY:*\n"
                f"• 📤 Ad Posts: {stats['posts_today']}\n"
                f"• 🛒 Purchases: {stats['purchases_today']}\n"
    ...
    async def admin_panel_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Admin panel for advertising bot (Admin only)"""
        if update.effective_user.id not in ADMIN_IDS:
            await update.message.reply_text("⛔ Only Supreme Admins can access the admin panel!")
            return
        ...
        admin_text = (
            ...
            "🔄 *System Control:*\n"
            "/adv_pause - Pause advertising\n"
            "/adv_resume - Resume advertising\n"
            "/adv_setinterval - Set post interval\n\n"
            
            "📋 *Ad Management:*\n"
            "/adv_viewqueue - View all ads\n"
            "/adv_removead - Remove specific ad\n"
            "/adv_forcead - Force post next ad\n"
            "/adv_free_ad - Create free ad\n\n"
            
            "📈 *Analytics:*\n"
            "/adv_stats - Advertising stats\n"
            "/adv_system_stats - System stats\n\n"
            
            "💫 *Your command is my duty, O Great Admin!*"
        )
        
        await update.message.reply_text(admin_text, parse_mode=ParseMode.MARKDOWN)
    
    @database_error_handler
    async def post_next_ad(self):
        """Post the next advertisement in queue"""
        try:
            async with db_manager.get_connection() as db:
                # Get ad config
                cursor = await db.execute("SELECT is_paused, post_interval FROM ad_config WHERE id=1")
                ...
                if not config_result:
                    logger.error("No ad configuration found")
                    return
                
                is_paused, post_interval = config_result
                
                if is_paused:
                    logger.info("Advertising is paused, skipping post")
                    return
                
                # Get next active ad
                cursor = await db.execute("""
                    SELECT * FROM ads_queue 
                ...
                if not ad_result:
                    logger.info("No active ads to post")
                    return
                
                # Parse ad data
                (ad_id, user_id, username, heading, ad_type, description, 
                 contact, created_at, expires_at, status, post_count, 
                 has_buttons, button_data_json, has_image, image_url) = ad_result
                ...
                ad_text = (
                    ...
                    f"💫 _This ad will expire on {datetime.fromisoformat(expires_at).strftime('%Y-%m-%d')}_"
                )
                
                # Prepare buttons if available
                reply_markup = None
                if has_buttons and button_data_json:
                    try:
                        ...
                        logger.error(f"Error parsing button data: {e}")
                ...
                try:
                    ...
                    await db.execute("""
                        INSERT INTO statistics (metric_name, metric_value, recorded_at)
                        VALUES (?, ?, ?)
                    """, ("ad_posted", 1, datetime.now().isoformat()))
                    
                    await db.commit()
                    
                    logger.info(f"✅ Posted ad #{ad_id} in main group")
                    
                except Exception as e:
                    logger.error(f"Error posting ad #{ad_id}: {e}")
                    # Mark problematic ad as inactive
                    await db.execute(
                        "UPDATE ads_queue SET status = 'error' WHERE id = ?",
                        (ad_id,)
                    )
                    await db.commit()
                    
        except Exception as e:
            logger.error(f"Error in post_next_ad: {e}")
            await ConfigManager.log_error("advertising_bot", "post_next_ad", str(e), traceback.format_exc())
    ...
    def setup_handlers(self):
        ...
        self.app.add_handler(CommandHandler(f"{self.prefix}_viewqueue", self.viewqueue_command))
        self.app.add_handler(CommandHandler(f"{self.prefix}_free_ad", self.free_ad_command))
        self.app.add_handler(CommandHandler(f"{self.prefix}_admin", self.admin_panel_command))
        
        # Admin commands
        self.app.add_handler(CommandHandler(f"{self.prefix}_pause", self.pause_command))
        self.app.add_handler(CommandHandler(f"{self.prefix}_resume", self.resume_command))
        self.app.add_handler(CommandHandler(f"{self.prefix}_removead", self.removead_command))
        self.app.add_handler(CommandHandler(f"{self.prefix}_setinterval", self.setinterval_command))
        self.app.add_handler(CommandHandler(f"{self.prefix}_forcead", self.forcead_command))
        self.app.add_handler(CommandHandler(f"{self.prefix}_system_stats", self.system_stats_command))
        
        # Message handlers for admin ad creation
        self.app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, self.handle_admin_ad_creation))
        
        # Callback handlers for admin ad creation
        self.app.add_handler(CallbackQueryHandler(self.handle_admin_ad_callback, pattern="^admin_"))
        
        # Add global error handler
        self.app.add_error_handler(self.global_error_handler)
    
    async def setup_scheduler(self):
        """Setup the advertisement scheduler"""
        try:
    ...
    async def start_bot(self):
        """Start the advertising bot"""
        try:
            await init_database()
            self.setup_handlers()
            
            # Start scheduler for posting ads
            await self.setup_scheduler()
            
            # Start the bot using proper polling
            await self.app.initialize()
            await self.app.start()
            await self.app.updater.start_polling()
            
            self.is_running = True
            logger.info(f"✅ Advertising Bot started successfully with prefix: {self.prefix}")
            
        except Exception as e:
            logger.error(f"❌ Failed to start Advertising Bot: {e}")
            raise
    
    async def stop_bot(self):
        """Stop the advertising bot"""
        try:
            ...
            self.is_running = False
            logger.info("✅ Advertising Bot stopped successfully")
            
        except Exception as e:
            logger.error(f"Error stopping Advertising Bot: {e}")
    ...
    async def run(self):
            ...
            try:
                ...
                while self.is_running:
                    try:
                        ...
                        logger.error(f"Error in advertising bot main loop: {e}")
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.error("❌ All advertising bot startup attempts failed.")
                    return
        ...
        try:
            ...
            logger.error(f"Error during advertising bot shutdown: {e}")
...
# ============================
# 🤖 4. AUTO ADV BOT - COMPLETE FIXED VERSION WITH ENHANCED SCAM REPORTS
# ============================
...
"""
    ...
    async def start_scammer_report(self, query, context, user_id):
        """Start scammer report flow - ENHANCED FOR MULTIPLE IMAGES"""
        self.user_states[user_id] = {'product': 'scammer', 'step': 1, 'data': {}, 'images': []}
        ...
        await query.edit_message_text(
            ...
            "━━━━━━━━━━━━━━━━\n"
            "📝 *Step 1 of 7: Enter Scammer Details*\n\n"
    ...
    async def handle_scammer_flow(self, update: Update, context: ContextTypes.DEFAULT_TYPE, state: Dict, message: str):
        """Handle scammer report flow - ENHANCED FOR MULTIPLE IMAGES"""
        user_id = update.effective_user.id
        ...
        if state['step'] == 1:  # Scammer details
            ...
            await update.message.reply_text(
                "✅ *Evidence saved!*\n\n"
                "━━━━━━━━━━━━━━━━\n"
                "📝 *Step 3 of 7: Enter Your Contact*\n\n"
            ...
            await update.message.reply_text(
                "✅ *Contact saved!*\n\n"
                "━━━━━━━━━━━━━━━━\n"
                "📝 *Step 4 of 7: Scammer's USDT Address*\n\n"
                "If you have the scammer's USDT address, please provide it:\n"
                "_(If unknown, type 'Unknown')_",
                parse_mode=ParseMode.MARKDOWN
            )
            
        elif state['step'] == 4:  # USDT Address
            state['data']['scammer_usdt_address'] = message
            state['step'] = 5
            
            await update.message.reply_text(
                "✅ *USDT address saved!*\n\n"
                "━━━━━━━━━━━━━━━━\n"
                "📝 *Step 5 of 7: Communication Screenshots*\n\n"
                "📸 *NOW YOU CAN UPLOAD MULTIPLE IMAGES!*\n\n"
                "Please upload all relevant screenshots of your communication with the scammer.\n"
                "You can send multiple images one by one.\n\n"
                "When you're done uploading images, type /done to proceed to the next step.\n\n"
                "📎 *Supported:* Photos, screenshots, documents\n"
                "⚡ *Maximum:* 10 images per report\n\n"
                "Start uploading your images now:",
                parse_mode=ParseMode.MARKDOWN
            )
            
        elif state['step'] == 6:  # Final confirmation
            # This step is handled by handle_done_command
            pass
    
    @error_handler
    async def handle_scammer_image_upload(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Handle multiple image uploads for scammer reports - ENHANCED VERSION"""
        user_id = update.effective_user.id
        
        # Check if user is in scammer report flow
        if user_id not in self.user_states:
            return
        
        state = self.user_states[user_id]
        
        if state['product'] != 'scammer' or state['step'] != 5:
            return
        ...
        if len(state['images']) >= 10:
            await update.message.reply_text(
                ...
                "You have reached the maximum of 10 images per report.\n"
                "Type /done to proceed to submission."
            )
            return
        ...
        if update.message.photo:
            ...
            # Store file_id in images list
            state['images'].append(file_id)
            
            image_count = len(state['images'])
            
            await update.message.reply_text(
                f"✅ *Image #{image_count} received!*\n\n"
                f"You have uploaded {image_count}/10 images.\n"
                f"Continue uploading more images or type /done to proceed.",
                parse_mode=ParseMode.MARKDOWN
            )
        
        # Handle document (screenshot)
            ...
            if document.mime_type and document.mime_type.startswith('image/'):
                ...
                await update.message.reply_text(
                    f"✅ *Document image #{image_count} received!*\n\n"
                    f"You have uploaded {image_count}/10 images.\n"
                    f"Continue uploading more images or type /done to proceed.",
                    parse_mode=ParseMode.MARKDOWN
                )
                ...
                await update.message.reply_text(
                    "❌ Please upload only images (photos or screenshots)."
                )
        
        else:
            await update.message.reply_text(
                "❌ Please upload images only. Send photos or image documents."
            )
    ...
    async def handle_done_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if user_id not in self.user_states:
            await update.message.reply_text("This command is only available during purchase flows.")
            return
            
        state = self.user_states[user_id]
        
        if state['product'] == 'scammer' and state['step'] == 5:
            image_count = len(state.get('images', []))
            
            if image_count == 0:
                await update.message.reply_text(
                    "⚠️ *NO IMAGES UPLOADED*\n\n"
                    "You haven't uploaded any images. Are you sure you want to continue without evidence?\n\n"
                    "Type /done again to confirm, or continue uploading images.",
                    parse_mode=ParseMode.MARKDOWN
                )
                state['confirm_no_images'] = True
                return
            
            if state.get('confirm_no_images'):
                # User confirmed no images
                state['confirm_no_images'] = False
                await self.finalize_scammer_report(update, context, state)
            else:
                # Proceed with images
                await self.finalize_scammer_report(update, context, state)
    
    @error_handler
    async def finalize_scammer_report(self, update: Update, context: ContextTypes.DEFAULT_TYPE, state: Dict):
        """Finalize scammer report with multiple images"""
        user_id = update.effective_user.id
        image_count = len(state.get('images', []))
        
        state['step'] = 6
        
        # Store images in state data
        state['data']['communication_screenshots'] = state.get('images', [])
        
        report_summary = f"""
✅ *SCAMMER REPORT READY FOR SUBMISSION!* ✅

📋 *Report Summary:*
• Scammer Details: {state['data']['scammer_details'][:100]}...
• Evidence: {state['data']['evidence'][:100]}...
• Contact: {state['data']['contact']}
• USDT Address: {state['data']['scammer_usdt_address']}
• Images Uploaded: {image_count}

🚨 *This report will be sent to admins for review and immediate action.*

Click 'Submit Report' to send it for admin approval:
"""
        
        keyboard = [
            [InlineKeyboardButton("🚨 Submit Report", callback_data="submit_scammer_report")],
            [InlineKeyboardButton("❌ Cancel", callback_data="cancel_scammer_report")]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
    ...
    async def handle_payment_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Handle payment-related callbacks - FIXED VERSION"""
        query = update.callback_query
        await query.answer()
        ...
        if callback_data == "buttons_yes":
            ...
            # FIX: Directly initiate payment when user selects "No" for image
            await self.initiate_payment(update, context, state)
        
        elif callback_data == "submit_scammer_report":
            await self.submit_scammer_report(update, context, state)
        
        elif callback_data == "cancel_scammer_report":
            await query.edit_message_text(
                "❌ Scammer report cancelled.\n\n"
                "No information has been saved. You can start over with /start"
            )
            if user_id in self.user_states:
                del self.user_states[user_id]
    ...
    async def initiate_payment(self, update: Update, context: ContextTypes.DEFAULT_TYPE, state: Dict):
        """Initiate payment process - FIXED: Skip verification in dummy mode"""
        user_id = update.effective_user.id
        ...
        # FIX: In dummy mode, automatically process payment without verification
        if PAYMENT_MODE == "dummy":
            await self.process_dummy_payment_auto(update, context, user_id, state['product'], amount, state['data'], payment_id)
            return
        
        # Only show payment instructions in real mode
        payment_text = f"""
...
"""
    ...
    @error_handler
    async def process_dummy_payment_auto(self, update: Update, context: ContextTypes.DEFAULT_TYPE, 
                                       user_id: int, product: str, amount: float, data: Dict, payment_id: str):
        """Process dummy payment automatically without verification - FIXED VERSION"""
        # Simulate successful verification
        tx_hash = f"DUMMY_TX_{int(datetime.now().timestamp())}"
        
        # Record purchase directly
        async with db_manager.get_connection() as db:
            await db.execute("""
    ...
    async def verify_payment_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Handle payment verification callback - Only used in REAL mode"""
        query = update.callback_query
        await query.answer()
        
        # Only process verification in real mode
        if PAYMENT_MODE == "dummy":
            await query.edit_message_text(
                "❌ *Payment verification is disabled in dummy mode.*\n\n"
                "Switch to REAL mode for payment processing.",
                parse_mode=ParseMode.MARKDOWN
            )
            return
    ...
    async def handle_tx_hash_input(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Handle transaction hash input - Only used in REAL mode"""
        if PAYMENT_MODE == "dummy":
            await update.message.reply_text(
                "❌ *Payment verification is disabled in dummy mode.*\n\n"
                "Switch to REAL mode for payment processing.",
...
"""
    ...
    async def submit_scammer_report(self, update: Update, context: ContextTypes.DEFAULT_TYPE, state: Dict):
        """Submit scammer report for admin approval"""
        await self.finalize_purchase(update, context, state)
    
    @error_handler
    async def process_scammer_report(self, update: Update, context: ContextTypes.DEFAULT_TYPE, data: Dict, user_id: int):
        """Process scammer report - SEND TO ADMIN DM FOR APPROVAL - ENHANCED FOR MULTIPLE IMAGES"""
        # Get image file URLs if available
        image_urls = []
        image_file_ids = data.get('communication_screenshots', [])
        ...
        if image_file_ids:
            for file_id in image_file_ids:
                try:
                    ...
                    logger.warning(f"Could not get file URL for {file_id}: {e}")
                    # Store file_id as fallback
                    image_urls.append(f"file_id:{file_id}")
        
        # Save to database for admin approval
        async with db_manager.get_connection() as db:
            cursor = await db.execute("""
                INSERT INTO scammer_reports 
                (user_id, username, scammer_details, evidence, contact, communication_screenshots, scammer_usdt_address, status, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, 'pending', ?)
            """, (
                user_id,
                ...
                data['evidence'],
                data['contact'],
                json.dumps(image_urls),  # Store as JSON array
                data.get('scammer_usdt_address', ''),
                datetime.now().isoformat()
            ))
            report_id = cursor.lastrowid
            await db.commit()

        # Create comprehensive report message for admin approval
        image_count = len(image_urls)
        image_info = f" with {image_count} image(s)" if image_count > 0 else ""
        
        report_message = f"""
🚨 *NEW SCAMMER REPORT - REQUIRES APPROVAL* 🚨

📋 *Report ID:* #{report_id}
👤 *Reporter:* @{update.effective_user.username or 'Unknown'} (ID: {user_id})
...
💰 *Scammer USDT Address:*
`{data.get('scammer_usdt_address', 'Not provided')}`

📞 *Reporter Contact:*
{data['contact']}

🖼️ *Communication Proof:*
{'✅ ' + str(image_count) + ' screenshot(s) provided' if image_count > 0 else '❌ No screenshots'}
"""

        # Send to all admins in DM with approval buttons
        for admin_id in ADMIN_IDS:
            try:
                keyboard = [
                    [
                        InlineKeyboardButton("✅ Approve Report", callback_data=f"approve_scam_{report_id}"),
                        InlineKeyboardButton("❌ Reject Report", callback_data=f"reject_scam_{report_id}")
                    ],
                    [
                        InlineKeyboardButton("📋 View All Pending", callback_data="view_pending_scams")
                    ]
                ]
                reply_markup = InlineKeyboardMarkup(keyboard)
                
                # If there are images, send the first one with the report
                if image_urls:
                    try:
                        first_image = image_urls[0]
                        if first_image.startswith('file_id:'):
                            # Use file_id directly
                            file_id = first_image.replace('file_id:', '')
                        ...
                        for i, image_url in enumerate(image_urls[1:], 2):
                            try:
                                if image_url.startswith('file_id:'):
                                    file_id = image_url.replace('file_id:', '')
                                    await self.app.bot.send_photo(
                                        chat_id=admin_id,
                                        photo=file_id,
                                        caption=f"📸 Image {i} of {image_count} - Report #{report_id}"
                                    )
                                else:
                                    await self.app.bot.send_photo(
                                        chat_id=admin_id,
                                ...
                                logger.error(f"Error sending additional image {i}: {e}")
                        ...
                        await self.app.bot.send_message(
                            chat_id=admin_id,
                            text=report_message + f"\n\n❌ Could not send images: {e}",
                            reply_markup=reply_markup,
                            parse_mode=ParseMode.MARKDOWN
                        )
                else:
                ...
                logger.info(f"✅ Scammer report #{report_id}{image_info} sent to admin {admin_id} for approval")
                
            except Exception as e:
                logger.error(f"❌ Could not send scam report to admin {admin_id}: {e}")

        success_text = f"""
✅ *SCAMMER REPORT SUBMITTED FOR APPROVAL!*{image_info}

🚨 *Your report has been received and sent to admins for review!*

📋 *What happens next:*
• Admin reviews your evidence
• Verification process
• If approved, instantly posted to scammer channel
• Community alerted and protected
...
"""
    ...
    @error_handler
    async def handle_scam_approval_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Handle scam report approval/rejection by admin - ENHANCED FOR MULTIPLE IMAGES"""
        query = update.callback_query
        await query.answer()
        
        user_id = query.from_user.id
        if user_id not in ADMIN_IDS:
            await query.answer("⛔ Only admins can approve scam reports!", show_alert=True)
            return
        
        callback_data = query.data
        report_id = int(callback_data.split("_")[2])
        
        if callback_data.startswith("approve_scam_"):
            await self.approve_scam_report(query, context, report_id, user_id)
        elif callback_data.startswith("reject_scam_"):
            await self.reject_scam_report(query, context, report_id, user_id)
        elif callback_data == "view_pending_scams":
            await self.view_pending_scam_reports(query, context)
    
    @error_handler
    async def approve_scam_report(self, query, context, report_id: int, admin_id: int):
        """Approve a scam report and post to scammer channel - ENHANCED FOR MULTIPLE IMAGES"""
        # Get report details from database
        async with db_manager.get_connection() as db:
            cursor = await db.execute("""
                SELECT username, scammer_details, evidence, scammer_usdt_address, communication_screenshots
                FROM scammer_reports WHERE id=? AND status='pending'
            """, (report_id,))
            ...
            reporter_username, scammer_details, evidence, usdt_address, screenshots_json = report
            
            # Parse screenshots
            image_urls = []
            if screenshots_json:
                try:
                    image_urls = json.loads(screenshots_json)
                except:
                    pass
            ...
            await db.execute("""
                ...
                WHERE id=?
            """, (datetime.now().isoformat(), admin_id, report_id))
            await db.commit()
        
        # Create post for scammer channel
        image_count = len(image_urls)
        image_info = f" ({image_count} evidence image(s))" if image_count > 0 else ""
        
        scammer_post = f"""
🚨 *SCAMMER ALERT* 🚨

⚠️ *Verified Scammer Report*{image_info}
...
💰 *Scammer USDT Address:*
`{usdt_address or 'Not provided'}`

👤 *Reported by:* @{reporter_username}
✅ *Verified by:* Admin
📅 *Posted:* {datetime.now().strftime('%Y-%m-%d %H:%M')}

🔒 *Stay safe! Always verify before trading.*
"""
        
        # Post to scammer channel - WITH MULTIPLE IMAGES IF AVAILABLE
        try:
            if image_urls:
                        ...
                        try:
                            if image_url.startswith('file_id:'):
                                ...
                                await self.app.bot.send_photo(
                                    chat_id=SCAMMER_EXPOSED_ID,
                                    photo=file_id,
                                    caption=f"📸 Evidence image {i} of {image_count} - Report #{report_id}"
                                )
                            else:
                                await self.app.bot.send_photo(
                                    chat_id=SCAMMER_EXPOSED_ID,
                            ...
                            logger.error(f"Error sending additional evidence image {i}: {e}")
                            
                except Exception as e:
                    logger.error(f"Error sending photo to scammer channel: {e}")
                    # Fallback to text only
                    message = await self.app.bot.send_message(
                        chat_id=SCAMMER_EXPOSED_ID,
                        text=scammer_post + f"\n\n📎 *{image_count} evidence images available in admin records*",
                        parse_mode=ParseMode.MARKDOWN
                    )
            else:
                # Send text only
            ...
            try:
                await self.app.bot.send_message(
                    chat_id=user_id,  # Use the reporter's user_id
                    text=f"✅ *Your scammer report has been APPROVED and posted!*\n\n"
                         f"📸 Evidence images: {image_count}\n"
                         f"🚨 Now visible in scammer alerts channel\n\n"
                         f"Thank you for protecting the community! 🛡️",
                    parse_mode=ParseMode.MARKDOWN
                )
            except Exception as e:
                logger.warning(f"Could not notify reporter {reporter_username}: {e}")
            
            await query.edit_message_text(
                f"✅ *Scam Report #{report_id} APPROVED!*\n\n"
                f"Posted to scammer channel successfully!\n"
                f"📸 Images: {image_count}\n"
                f"Reporter has been notified.",
                parse_mode=ParseMode.MARKDOWN
            )
            
        except Exception as e:
            logger.error(f"Error posting to scammer channel: {e}")
            await query.edit_message_text(
                f"❌ Error posting to scammer channel: {e}",
                parse_mode=ParseMode.MARKDOWN
            )
    
    @error_handler
    async def reject_scam_report(self, query, context, report_id: int, admin_id: int):
        """Reject a scam report"""
        async with db_manager.get_connection() as db:
            # Get reporter info for notification
            cursor = await db.execute("SELECT user_id, username FROM scammer_reports WHERE id=?", (report_id,))
            result = await cursor.fetchone()
            reporter_user_id = result[0]
            reporter_username = result[1]
        ...
        try:
            await self.app.bot.send_message(
                ...
                text=f"❌ *Your scammer report #{report_id} was rejected.*\n\nPlease provide more evidence or contact support for details.",
                parse_mode=ParseMode.MARKDOWN
            )
        except Exception as e:
            logger.warning(f"Could not notify reporter {reporter_username}: {e}")
        ...
        await query.edit_message_text(
            f"❌ *Scam Report #{report_id} REJECTED!*\n\n"
            f"Reporter has been notified (if possible).",
            parse_mode=ParseMode.MARKDOWN
        )
    
    @error_handler
    async def view_pending_scam_reports(self, query, context):
        """Show all pending scam reports to admin"""
        async with db_manager.get_connection() as db:
            cursor = await db.execute("""
                SELECT id, username, scammer_details, created_at, communication_screenshots
                FROM scammer_reports 
                WHERE status='pending' 
                ORDER BY created_at DESC
            """)
            pending_reports = await cursor.fetchall()
        
        if not pending_reports:
            await query.edit_message_text("✅ No pending scam reports!")
            return
        
        reports_text = "📋 *PENDING SCAM REPORTS* 📋\n\n"
        
        for report in pending_reports:
            report_id, username, details, created_at, screenshots_json = report
            ...
            reports_text += f"👤 Reporter: @{username}\n"
            reports_text += f"📅 Submitted: {created.strftime('%m/%d %H:%M')}\n"
            reports_text += f"📸 Images: {image_count}\n"
            reports_text += f"🔍 Details: {details[:50]}...\n"
            reports_text += "━━━━━━━━━━━━━━━━\n\n"
            
            # Add quick action buttons for each report
            keyboard = [
                [
                    InlineKeyboardButton(f"✅ Approve #{report_id}", callback_data=f"approve_scam_{report_id}"),
                    InlineKeyboardButton(f"❌ Reject #{report_id}", callback_data=f"reject_scam_{report_id}")
                ]
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
            ...
            reports_text = ""  # Reset for next message
...
"""
    ...
    def setup_handlers(self):
        ...
        try:
            ...
            self.app.add_handler(CommandHandler("done", self.handle_done_command))
            ...
            self.app.add_handler(CallbackQueryHandler(self.handle_payment_callback, pattern="^submit_scammer_report$"))
            self.app.add_handler(CallbackQueryHandler(self.handle_payment_callback, pattern="^cancel_scammer_report$"))
            
            # Add scam report approval handlers
            self.app.add_handler(CallbackQueryHandler(
                self.handle_scam_approval_callback, 
                pattern="^(approve_scam_|reject_scam_|view_pending_scams)"
            ))
            ...
            # Image handler for scammer reports - ENHANCED: Now handles multiple images
            self.app.add_handler(MessageHandler(
                filters.PHOTO | filters.Document.IMAGE,
                self.handle_scammer_image_upload
            ))
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info("🔧 Version: 1.4.4 - Enhanced Multi-Image Support for Scammer Reports + All Features Restored")
