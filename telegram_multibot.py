...
Author: Claude  
Version: 1.4.1 - Fixed Image Upload for Scammer Reports + All Bots
"""
...
"""
    ...
    async def handle_scammer_flow(self, update: Update, context: ContextTypes.DEFAULT_TYPE, state: Dict, message: str):
        ...
        if state['step'] == 1:  # Scammer details
        ...
        elif state['step'] == 4:  # Communication screenshot
            if update.message.photo:
                # Get the largest photo
                photo = update.message.photo[-1]
                file = await photo.get_file()
                state['data']['communication_screenshot'] = file.file_path
            else:
                state['data']['communication_screenshot'] = message
            ...
            await update.message.reply_text(
                "✅ *Screenshot received!*\n\n"
                "━━━━━━━━━━━━━━━━\n"
                "📝 *Step 5 of 6: Scammer USDT Address*\n\n"
                "Please provide the scammer's USDT address (TRC20):",
                parse_mode=ParseMode.MARKDOWN
            )
    ...
    async def handle_scammer_image_upload(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Handle image uploads for scammer reports - FIXED VERSION"""
        user_id = update.effective_user.id
        
        # Get current report state
        scammer_data = context.user_data.get('scammer_report', {})
        current_step = scammer_data.get('step', 1)
        
        # Check if we're at the communication proof step
        if current_step == 4 and update.message.photo:
            ...
            # Store file_id in the scammer data
            scammer_data['communication_screenshot'] = f"telegram_file_id:{file_id}"
            scammer_data['step'] = 5
            
            # Update context
            context.user_data['scammer_report'] = scammer_data
            ...
            # Handle other file types or unexpected states
            await update.message.reply_text(
                "📎 Please continue with the report by providing the requested information in text format, "
                "or use the 'skip' command to proceed to the next step.",
                parse_mode=ParseMode.MARKDOWN
            )
...
"""
    ...
    async def process_scammer_report(self, update: Update, context: ContextTypes.DEFAULT_TYPE, data: Dict, user_id: int):
        """Process scammer report - SEND TO ADMIN DM FOR APPROVAL"""
...
{data.get('communication_screenshot', 'Not provided')}
"""
...
"""
    ...
    async def handle_scam_approval_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Handle scam report approval/rejection by admin"""
        query = update.callback_query
        await query.answer()
    ...
    async def approve_scam_report(self, query, context, report_id: int, admin_id: int):
        """Approve a scam report and post to scammer channel"""
        # Get report details from database
        async with db_manager.get_connection() as db:
            cursor = await db.execute("""
                SELECT username, scammer_details, evidence, scammer_usdt_address
            ...
            reporter_username, scammer_details, evidence, usdt_address = report
...
"""
        ...
        try:
            ...
            try:
                await self.app.bot.send_message(
                    chat_id=reporter_username,  # This should be user_id in production
                    text=f"✅ *Your scammer report has been APPROVED and posted!*\n\nThank you for protecting the community! 🛡️",
                    parse_mode=ParseMode.MARKDOWN
                )
            except:
                logger.warning(f"Could not notify reporter {reporter_username}")
    ...
    async def reject_scam_report(self, query, context, report_id: int, admin_id: int):
        ...
        async with db_manager.get_connection() as db:
            ...
            cursor = await db.execute("SELECT username FROM scammer_reports WHERE id=?", (report_id,))
            reporter_username = (await cursor.fetchone())[0]
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info("🔧 Version: 1.4.1 - AutoADV Bot Flow Fixed + Admin Free Ads + Enhanced Scammer Reports + Fixed Image Upload")
