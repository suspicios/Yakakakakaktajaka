...
async def create_ad_keyboard():
    ...
    keyboard = [
        [InlineKeyboardButton("📢 Post Your Ad", url=f"t.me/YOUR_AUTOADV_BOT?start=buy_ad")],
        [InlineKeyboardButton("⚠️ Report Scammer", url=f"t.me/YOUR_AUTOADV_BOT?start=report_scammer")],
        [InlineKeyboardButton("💎 Get VIP Access", url=f"t.me/YOUR_AUTOADV_BOT?start=buy_vip")],
...
Ad ID: #{ad['id']} | Valid until: {ad['expiry'].strftime('%Y-%m-%d')}
    ...
    def run(self):
        ...
        asyncio.create_task(self.broadcast_ads())
