...
"""
                    keyboard = [
                        [InlineKeyboardButton("📢 Post Your Ad", url=f"https://t.me/YourAutoADVBot?start=buy_ad")],
                        [InlineKeyboardButton("⚠️ Report Scammer", url=f"https://t.me/YourAutoADVBot?start=report_scammer")]
                    ]
...
"""
                    keyboard = [
                        [InlineKeyboardButton("💎 Join VIP", url=f"https://t.me/c/{str(VIP_CHANNEL_ID)[4:]}")],
                        [InlineKeyboardButton("🏢 Company Resources", url=f"https://t.me/c/{str(COMPANY_RESOURCES_ID)[4:]}")],
                        [InlineKeyboardButton("📢 Post Ad", url=f"https://t.me/YourAutoADVBot?start=buy_ad")],
                        [InlineKeyboardButton("⚠️ Report Scammer", url=f"https://t.me/YourAutoADVBot?start=report_scammer")]
                    ]
...
"""
    ...
    async def new_member_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        for member in update.message.new_chat_members:
            ...
            keyboard = [
                [InlineKeyboardButton("✅ Main Group", url=f"https://t.me/c/{str(MAIN_GROUP_ID)[4:]}")],
                [InlineKeyboardButton("💎 VIP Channel", url=f"https://t.me/c/{str(VIP_CHANNEL_ID)[4:]}")],
                [InlineKeyboardButton("🏢 Company Resources", url=f"https://t.me/c/{str(COMPANY_RESOURCES_ID)[4:]}")],
                [InlineKeyboardButton("⚠️ Scammer Exposed", url=f"https://t.me/c/{str(SCAMMER_EXPOSED_ID)[4:]}")],
    ...
    async def kick_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not update.message.reply_to_message:
            await update.message.reply_text("📝 Reply to user's message to kick them")
            return
...
"""
    ...
    async def message_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text(
            ...
            f"✅ Priority support\n"
            f"✅ Exclusive access\n\n"
            f"🚀 Contact @YourAutoADVBot to upgrade!",
            parse_mode=ParseMode.MARKDOWN
        )
...
class GroupManagementBot:
    ...
    async def help_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Help command"""
        is_admin = update.effective_user.id in ADMIN_I, update.message.text or ""):
                try:
                    await update.message.delete()
    ...
    async def handle_ad_flow(self, update, context, user_id, step, user_input):
        ...
        if step == 1:
            ...
            if PAYMENT_MODE == "dummy":
                ...
                await update.message.reply_text(
                    ...
                    f"💰 Amount: 188 USDT (TRC20)\n"
                    f"📍 Address:\n`{YOUR_USDT_ADDRESS}`\n\n"
                    f"📝 *Steps:*\n"
                    f"1️⃣ Send exactly 188 USDT to the address above\n"
        ...
        elif step == 5 and state.get('awaiting_payment'):
            if PAYMENT_MODE == "dummy" and user_input.lower() == "paid":
                await self.finalize_purchase(update, context, user_id)
            elif PAYMENT_MODE == "real":
    ...
    async def handle_vip_flow(self, update, context, user_id, step, user_input):
        ...
        if step == 1:
            ...
            if PAYMENT_MODE == "dummy":
                ...
                await update.message.reply_text(
                    f"💳 *PAYMENT INSTRUCTIONS* 💳\n\n"
                    f"💰 Amount: 300 USDT (TRC20)\n"
                    f"📍 Address:\n`{YOUR_USDT_ADDRESS}`\n\n"
                    f"📝 *Steps:*\n"
                    f"1️⃣ Send exactly 300 USDT to the address above\n"
                    f"2️⃣ Copy your transaction hash from TronScan\n"
                    f"3️⃣ Send the transaction hash here\n\n"
                    f"⏰ Verification takes 1-2 minutes!\n"
                    f"🔒 Your payment is 100% secure!",
        ...
        elif step == 4 and state.get('awaiting_payment'):
            if PAYMENT_MODE == "dummy" and user_input.lower() == "paid":
                await self.finalize_purchase(update, context, user_id)
...
if __name__ == "__main__":
    print("\n" + "="*65)
    print("           🌟 INTERLINK MULTI-BOT SYSTEM 🌟")
    print("="*65)
    print("\n  📢 Advertising Bot      - Auto-posting & promotion")
    print("  💎 VIP Bot              - Member verification")
    print("  🛡️  Group Management Bot - Spam control & moderation")
    print("  💰 Auto ADV Bot         - Payment processing")
    print(f"\n  Version: 1.0.1")
    print(f"  Payment Mode: {PAYMENT_MODE.upper()}")
    print("\n" + "="*65 + "\n")
...
class GroupManagementBot:
    ...
    async def help_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Help command"""
        is_admin = update.effective_user.id in ADMIN_I
