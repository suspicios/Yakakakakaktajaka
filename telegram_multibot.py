...
class AdvertisingBot:
    ...
    async def stop_advertising(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        if update.effective_user.id in [Config.MAIN_GROUP_ID]:  # Add admin IDs
...
async def run_all_bots():
    ...
    await asyncio.gather(
        adv_bot.app.run_polling(),
        vip_bot.app.run_polling(),
        group_bot.app.run_polling(),
        autoadv_bot.app.run_polling()
    )


if __name__ == "__main__":
    print("🤖 Starting Telegram Multi-Bot System...")
    print("⚠️  Remember to update Config class with your tokens and group IDs!")
