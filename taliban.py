...
Author: Claude  
Version: 1.3.0 - AutoADV Bot Flow Fixed + Enhanced Button System
"""
...
"""
    ...
    async def handle_ad_flow(self, update: Update, context: ContextTypes.DEFAULT_TYPE, state: Dict, message: str):
        ...
        if state['step'] == 1:  # Heading
            ...
            if len(message) > 500:
                await update.message.reply_text("❌ Description too long! Max 500 characters. Try again:")
                return
            ...
            await update.message.reply_text(
                "✅ *Description saved!*\n\n"
                "━━━━━━━━━━━━━━━━\n"
                "📝 *Step 4 of 8: Enter Contact Details*\n\n"
                "Please provide contact information:\n"
                "_(Phone, email, or username)_",
                parse_mode=ParseMode.MARKDOWN
            )
            ...
            await update.message.reply_text(
                ...
                "Would you like to add action buttons to your ad?\n\n"
                "🔘 *Example buttons:*\n"
                "• 'Contact Now'\n"
                "• 'Learn More'\n"
                "• 'Visit Website'",
        ...
        elif state['step'] == 5:  # Buttons configuration
            # This step is handled by callback
            pass
        
        elif state['step'] == 6:  # Button text input
            ...
            await update.message.reply_text(
                "✅ *Button text saved!*\n\n"
                "━━━━━━━━━━━━━━━━\n"
                "📝 *Step 7 of 8: Enter Button URL*\n\n"
                "Please provide the URL for your button:\n\n"
                "🌐 *Examples:*\n"
                "• https://t.me/username\n"
        ...
        elif state['step'] == 7:  # Button URL input
            # Validate URL format
            if not (message.startswith('http://') or message.startswith('https://') or message.startswith('tg://')):
                await update.message.reply_text("❌ Please provide a valid URL starting with http://, https://, or tg://")
                return
            ...
            await update.message.reply_text(
                ...
                "Would you like to add an image to your ad?",
                reply_markup=reply_markup,
                parse_mode=ParseMode.MARKDOWN
            )
        
        elif state['step'] == 8:  # Image URL (if yes)
    ...
    async def handle_scammer_flow(self, update: Update, context: ContextTypes.DEFAULT_TYPE, state: Dict, message: str):
        ...
        if state['step'] == 1:  # Scammer details
            ...
            await update.message.reply_text(
                ...
                "━━━━━━━━━━━━━━━━\n"
                "📝 *Step 2 of 3: Enter Proof*\n\n"
                "Please provide proof (screenshots, chat logs, etc.):\n"
                "_(You can send images/files)_",
                parse_mode=ParseMode.MARKDOWN
            )
        
        elif state['step'] == 2:  # Proof
            state['data']['proof'] = message
            state['step'] = 3
            
            await update.message.reply_text(
                "✅ *Proof details saved!*\n\n"
                "━━━━━━━━━━━━━━━━\n"
                "📝 *Step 3 of 3: Enter Your Report*\n\n"
                "Please write your full report:",
                parse_mode=ParseMode.MARKDOWN
            )
        
        elif state['step'] == 3:  # Report
            state['data']['report'] = message
    ...
    async def handle_payment_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if data == "buttons_yes":
            ...
            state['step'] = 6  # Move to button text input
            
            await query.edit_message_text(
                "✅ *Button preference saved!*\n\n"
                "━━━━━━━━━━━━━━━━\n"
                "📝 *Step 6 of 8: Enter Button Text*\n\n"
                "Please provide the text for your button:\n\n"
                "📝 *Examples:*\n"
                "• Contact Now\n"
                "• Learn More\n"
                "• Visit Website\n"
        ...
        elif data == "buttons_no":
            state['data']['has_buttons'] = False
            state['step'] = 8  # Skip to image step
            ...
            # Set step based on whether we have buttons or not
            if state['data'].get('has_buttons'):
                state['step'] = 9  # Image URL step after buttons
            else:
                state['step'] = 8  # Image URL step without buttons
        
        elif data == "image_no":
            state['data']['has_image'] = False
            state['data']['image_url'] = ""
            await self.initiate_payment(query, context, state)
    ...
    async def initiate_payment(self, update: Update, context: ContextTypes.DEFAULT_TYPE, state: Dict):
        ...
        # Prepare button data for storage
        button_data = []
        if state['product'] == 'ad' and state['data'].get('has_buttons'):
            if state['data'].get('button_text') and state['data'].get('button_url'):
                button_data.append({
                    'text': state['data']['button_text'],
                    'url': state['data']['button_url']
                })
        ...
        await self.show_payment_instructions(update, context, state, payment_id)
    
    @error_handler
    async def show_payment_instructions(self, update: Update, context: ContextTypes.DEFAULT_TYPE, state: Dict, payment_id: str):
...
1️⃣ Send *EXACTLY* {price} USDT
2️⃣ Use *TRC20* network only
3️⃣ Keep transaction hash ready
4️⃣ Payment auto-verifies in 2-5 minutes

⚠️ *DO NOT SEND OTHER TOKENS!*
⚠️ *WRONG AMOUNT = DELAYED ACTIVATION!*
...
"""
    ...
    async def verify_payment_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Check rate limiting
        if await self.check_rate_limit(user_id, "payment_verify"):
            await query.edit_message_text(
                "⏰ *Please wait 30 seconds between verification attempts!*",
                parse_mode=ParseMode.MARKDOWN
            )
            return
        ...
        # Verify transaction
        verified = await self.verify_transaction(user_id, amount)
        
        if verified:
            # Process successful payment
            await self.process_successful_payment(query, context, user_id, product, data, amount)
        else:
            await query.edit_message_text(
                "❌ *Payment not found!*\n\n"
                "Please check:\n"
                ...
                "Click 'Verify Payment' again after confirming.",
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("🔄 Verify Again", callback_data=f"verify_{payment_id}")
                ]]),
                parse_mode=ParseMode.MARKDOWN
            )
    ...
    async def check_rate_limit(self, user_id: int, action: str) -> bool:
        ...
        async with db_manager.get_connection() as db:
            ...
            if current_count >= 2:  # Max 2 attempts per minute
                return True
            
            # Log this attempt
            await db.execute("""
                INSERT INTO rate_limits (user_id, action, timestamp, count)
                VALUES (?, ?, ?, 1)
    ...
    async def verify_transaction(self, user_id: int, amount: float) -> bool:
        """Verify transaction on blockchain"""
        try:
            if PAYMENT_MODE == "dummy":
                # Dummy mode for testing - always return True
                logger.info(f"💰 Dummy payment verified for user {user_id}, amount {amount}")
                return True
            
            # Real payment verification using TronScan API
            async with aiohttp.ClientSession() as session:
                # Get recent transactions for our address
                async with session.get(f"{TRONSCAN_API}?address={YOUR_USDT_ADDRESS}&limit=50") as response:
                    if response.status == 200:
                        data = await response.json()
                        
                        # Look for matching transaction
                        for tx in data.get('data', []):
                            if (tx.get('to') == YOUR_USDT_ADDRESS.lower() and 
                                float(tx.get('amount', 0)) / 1e6 == amount and
                                tx.get('confirmed')):
                                ...
                                async with db_manager.get_connection() as db:
                                    cursor = await db.execute("""
                                        SELECT id FROM transaction_logs WHERE tx_hash=?
                                    """, (tx_hash,))
                                    if await cursor.fetchone():
                                ...
                                async with db_manager.get_connection() as db:
                                    await db.execute("""
                                        ...
                                        (user_id, tx_hash, verified, timestamp, details)
                                        VALUES (?, ?, 1, ?, ?)
                                    """, (user_id, tx_hash, datetime.now().isoformat(), json.dumps(tx)))
                                    await db.commit()
                                
                                logger.info(f"💰 Payment verified: {tx_hash} for user {user_id}")
                                return True
    ...
    @error_handler
    async def process_successful_payment(self, query, context, user_id: int, product: str, data: Dict, amount: float):
        """Process successful payment - FIXED VERSION"""
        username = query.from_user.username or "Unknown"
        
        if product == 'ad':
            # Create ad entry with button data
            expires_at = datetime.now() + timedelta(days=10)
            button_data_json = json.dumps(data.get('button_data', []))
            
            async with db_manager.get_connection() as db:
                await db.execute("""
                    INSERT INTO ads_queue 
                    (user_id, username, heading, type, description, contact, created_at, expires_at, 
                     has_buttons, button_data, has_image, image_url)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    user_id,
                    username,
...
✅ *Payment Verified:* {amount} USDT
📢 *Ad Status:* ACTIVE
⏰ *Valid Until:* {expires_at.strftime('%d/%m/%Y')}
...
• Buttons: {'✅ Enabled' if data.get('has_buttons') else '❌ Disabled'}
• Image: {'✅ Included' if data.get('has_image') else '❌ Not included'}

🚀 *Your ad will start appearing immediately!*
📌 Auto-posting every 5-6 minutes
📍 Pinned in main group
...
"""
        ...
        async with db_manager.get_connection() as db:
            ...
            """, (
                ...
                product,
                amount,
                f"verified_{int(datetime.now().timestamp())}",
                datetime.now().isoformat(),
                json.dumps(data)
            ))
            await db.commit()
        ...
        # Update statistics
        await ConfigManager.update_statistics(f"{product}_purchased")
        await ConfigManager.update_statistics("revenue_generated", int(amount))
...
🕵️ *Scammer details:* {state['data']['scammer_details']}
📋 *Report:* {state['data']['report']}
🔍 *Proof:* {state['data']['proof']}

━━━━━━━━━━━━━━━━
⚠️ *WARNING: Be cautious when dealing with this person!*

📢 *Verified by AutoADV Bot*
"""
...
━━━━━━━━━━━━━━━━
🔒 *Note:* All reports are verified before posting to ensure fairness.
"""
...
There was an error posting your report. Please try again later or contact support.
...
"""
        ...
        purchases_text = "📊 *YOUR PURCHASE HISTORY* 📊\n\n"
        ...
        for purchase in purchases:
            ...
            purchases_text += f"🛍️ *{product_type.upper()}*\n"
        ...
        purchases_text += f"💵 *Total Spent:* {total_spent} USDT\n\n"
        purchases_text += "💫 *Thank you for your business!*"
...
1. Select a product from main menu
2. Follow the step-by-step process
3. Complete payment via USDT (TRC20)
...
🚨 *SUPPORT:*
For issues with payments or activation, contact support.
...
"""
    ...
    async def cancel_payment_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await query.edit_message_text(
            ...
            "🛍️ Feel free to browse our products anytime!",
            reply_markup=InlineKeyboardMarkup([[
                InlineKeyboardButton("🛍️ View Products", callback_data="view_products")
            ]]),
            parse_mode=ParseMode.MARKDOWN
    ...
    def setup_handlers(self):
        ...
        try:
            ...
            self.app.add_handler(CallbackQueryHandler(self.handle_payment_callback, pattern="^(buttons_|image_|proceed_payment)"))
            self.app.add_handler(CallbackQueryHandler(self.verify_payment_callback, pattern="^verify_"))
    ...
    async def run(self):
        """Run the AutoADV bot - FIXED VERSION"""
        max_retries = 3
        retry_delay = 5
...
class BotOrchestrator:
    ...
    async def start_all_bots(self):
        ...
        try:
            ...
            logger.info("🚀 Launching bot army...")
            
            # Start all bots as concurrent tasks
            for name, bot in self.bots.items():
                task = asyncio.create_task(self._run_bot_safely(name, bot))
                self.tasks.append(task)
                logger.info(f"🎯 {name.capitalize()} bot task created")
    ...
    async def _run_bot_safely(self, name: str, bot):
        """Run a bot with safety wrapper"""
        max_restarts = 5
        restart_delay = 10
        
        for attempt in range(max_restarts):
            try:
                logger.info(f"🤖 Starting {name} bot (Attempt {attempt + 1})...")
                await bot.run()
                break
                
            except Exception as e:
                logger.error(f"❌ {name} bot crashed (Attempt {attempt + 1}): {e}")
                await ConfigManager.log_error(name, "bot_crash", str(e), traceback.format_exc())
                
                if attempt < max_restarts - 1:
                    logger.info(f"🔄 Restarting {name} bot in {restart_delay} seconds...")
                    await asyncio.sleep(restart_delay)
                    restart_delay *= 2  # Exponential backoff
                else:
                    logger.error(f"💀 {name} bot failed permanently after {max_restarts} attempts")
                    break
        ...
        try:
            ...
            for name, bot in self.bots.items():
                try:
                    ...
                    logger.info(f"✅ {name.capitalize()} bot stopped")
                except Exception as e:
...
async def main():
    ...
    try:
        ...
        # Log critical error
        await ConfigManager.log_error("system", "critical_failure", str(e), traceback.format_exc())
