...
"""
    ...
    def run_bot(self):
        ...
        async def async_run():
            await init_database()
            self.setup_handlers()
            self.start_scheduler()  # FIXED: Added scheduler start
...
"""
    ...
    async def edit_purchase(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await query.edit_message_text(
            "🔧 *Purchase Editing*\n\n"
            "Your purchase has been cancelled for editing.\n\n"
            "You can start over with /autoadvbuy to create a new advertisement with different details.",
            parse_mode=ParseMode.MARKDOWN
        )
    ...
    def setup_handlers(self):
        ...
        # Enhanced purchase conversation handler - FIXED PATTERNS
        ad_purchase_conv = ConversationHandler(
            ...
            states={
                ...
                AD_CONFIRMATION: [
                    ...
                    CallbackQueryHandler(self.edit_purchase, pattern="^edit_purchase$"),  # FIXED: Added edit handler
        ...
        # Enhanced VIP purchase handlers - FIXED PATTERNS
        ...
        # Other purchase callbacks - FIXED PATTERNS
        ...
        self.app.add_handler(CallbackQueryHandler(self.cancel_purchase, pattern="^cancel_purchase$"))
...
# ============================
# 🛡️ 4. ENHANCED GROUP MANAGEMENT BOT
# ============================

class EnhancedGroupManagementBot:
    def __init__(self, token: str):
        self.token = token
        self.app = Application.builder().token(token).build()
    ...
    async def start_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text(
            "🛡️ *ENHANCED GROUP MANAGEMENT BOT* 🛡️\n\n"
            "I am the *Enhanced Group Management Bot* with advanced protection! "
            "I keep your groups safe and organized with divine precision.\n\n"
            "✨ *ENHANCED FEATURES:* ✨\n"
            "🛡️ Advanced member verification\n"
            "🚫 Auto-spam protection\n"
            "📊 Enhanced analytics\n"
            "🔒 Secure group management\n\n"
            "⚡ *ENHANCED COMMANDS:* ⚡\n"
            "/grouphelp - All enhanced commands\n"
            "/groupstats - Group statistics\n"
            "/grouprules - View group rules\n\n"
            "Your groups are in safe hands, O Protector! 🛡️",
            parse_mode=ParseMode.MARKDOWN
        )
    
    async def help_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Enhanced help command for group management"""
        help_text = """
🛡️ *ENHANCED GROUP MANAGEMENT BOT - COMMAND BIBLE* 🛡️
...
📱 *GENERAL COMMANDS:*
/groupstart - Start group management
/grouphelp - Enhanced command reference
/groupstats - Group statistics
/grouprules - View group rules
/groupreport - Report issues

👑 *ADMIN COMMANDS:*
/groupwarn [user] - Warn a user
/groupban [user] - Ban a user
/groupmute [user] - Mute a user
/groupunban [user] - Unban a user
/grouppurge - Clean up messages
/groupsetrules - Set group rules
/groupsetwelcome - Set welcome message
/groupexport - Export member data

🛡️ *Your groups are protected with enhanced security!*
"""
        await update.message.reply_text(help_text, parse_mode=ParseMode.MARKDOWN)
    ...
    def run_bot(self):
        """Run the enhanced group management bot"""
        import asyncio
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        ...
        async def async_run():
            ...
            logger.info("🛡️ Enhanced Group Management Bot is running...")
            await self.app.initialize()
            await self.app.start()
            await self.app.updater.start_polling()
        ...
        try:
            ...
            logger.info("🛑 Enhanced Group Management Bot stopped by user")
        except Exception as e:
            logger.error(f"❌ Error in Enhanced Group Management Bot: {e}")
        finally:
            loop.run_until_complete(self.app.stop())
            loop.run_until_complete(self.app.shutdown())
            loop.close()
...
def run_all_bots():
    ...
    def run_group_bot():
        """Run enhanced group management bot"""
        try:
            group_bot = EnhancedGroupManagementBot(GROUP_BOT_TOKEN)
            group_bot.run_bot()
        except Exception as e:
            logger.error(f"Group bot error: {e}")
    ...
    # Start Group Management Bot
    group_thread = threading.Thread(target=run_group_bot, daemon=True)
    group_thread.start()
    threads.append(group_thread)
    logger.info("✅ Enhanced Group Management Bot started")
    
    print("🎉 ALL ENHANCED BOTS STARTED SUCCESSFULLY!")
    print("🤖 Bots are now running in the background...")
    print("📱 Available Bots:")
    print(f"   📢 Advertising Bot: {ADV_BOT_USERNAME}")
    print(f"   👑 VIP Bot: {VIP_BOT_USERNAME}")
    print(f"   💰 AutoADV Bot: {AUTOADV_BOT_USERNAME}")
    print(f"   🛡️ Group Bot: {GROUP_BOT_USERNAME}")
    ...
    try:
        ...
        print("🛑 All bots stopped by user")

def main():
    """Main function - automatically run all bots"""
    print("🚀 ENHANCED INTERLINK BOT SYSTEM")
    print("=" * 50)
    print("🤖 Starting ALL enhanced bots automatically...")
    print("💾 Database initialized successfully")
    print("🔄 Starting all bot threads...")
    
    # Automatically run all bots
    run_all_bots()
