...
# Conversation states for ad purchase - EXPANDED
    ...
    async def post_advertisement(self):
        ...
        try:
            async with aiosqlite.connect(DB_NAME) as db:
                ...
                if ad:
                    # Post purchased ad with enhanced features
                    ad_text = f"""
🎯 *{ad[1]}*
...
"""
                    ...
                    # Create keyboard with optional button
                    keyboard = []
...
class AutoAdvPaymentBot:
    ...
    async def start_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Check if in group and redirect to DM
        if update.effective_chat.type != "private":
            await update.message.reply_text(
                "🤖 *Please use me in private messages for purchases!*\n\n"
                f"Click here to start: [AutoADV Bot](https://t.me/{AUTOADV_BOT_USERNAME.replace('@', '')})",
                parse_mode=ParseMode.MARKDOWN,
...
"""
    ...
    async def buy_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Start purchase process - ENHANCED with DM check"""
        # Check if in group and redirect to DM
        if update.effective_chat.type != "private":
            await update.message.reply_text(
                "🤖 *Please use me in private messages for purchases!*\n\n"
    ...
    async def pay_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if update.effective_chat.type != "private":
            await update.message.reply_text(
                "🤖 *Please use me in private messages for payments!*\n\n"
                f"Click here to start: [AutoADV Bot](https://t.me/{AUTOADV_BOT_USERNAME.replace('@', '')})",
                parse_mode=ParseMode.MARKDOWN,
                disable_web_page_preview=True
            )
    ...
    # ============================
    # ENHANCED PURCHASE CONVERSATION FLOW
    # ============================
    
    async def start_ad_purchase(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Start advertisement purchase conversation - ENHANCED with DM check"""
        query = update.callback_query
        await query.answer()
        ...
        await query.edit_message_text(
            ...
            "🎯 *Step 1/7: Advertisement Heading*\n"
    ...
    async def receive_ad_type(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await query.edit_message_text(
            ...
            "🎯 *Step 3/7: Advertisement Description*\n"
    ...
    async def receive_ad_description(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        keyboard = [
            [InlineKeyboardButton("✅ Yes, Add Image", callback_data="image_yes")],
        ...
        await update.message.reply_text(
            f"✅ *Description Saved!*\n\n"
            "🎯 *Step 4/7: Add Image to Advertisement*\n"
    ...
    async def receive_image_option(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if image_choice == 'yes':
            ...
            await query.edit_message_text(
                "✅ *Image Option Selected:* Yes\n\n"
                "🎯 *Step 5/7: Upload Advertisement Image*\n"
    ...
    async def receive_button_details(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Receive button details - handles both text and URL"""
        user_id = update.message.from_user.id
        ...
        if 'button_text' not in ad_data:
            ...
            if len(button_text) > 20:
                await update.message.reply_text("❌ Button text too long! Maximum 20 characters. Please try again:")
                return AD_BUTTON_DETAILS
            ...
            if not button_url.startswith(('http://', 'https://')):
                await update.message.reply_text("❌ Invalid URL! Must start with http:// or https://. Please try again:")
                return AD_BUTTON_DETAILS
    ...
    async def receive_ad_contact(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Show enhanced confirmation with all features
        ad_data = self.ad_purchase_data[user_id]
        ...
        if ad_data.get('has_image') == 1:
            if ad_data.get('image_file_id'):
                confirmation_text += "🖼️ *Image:* ✅ INCLUDED\n"
            else:
                confirmation_text += "🖼️ *Image:* ✅ READY FOR UPLOAD\n"
        else:
        ...
        confirmation_text += f"\n💰 *Price:* $50.00 (7 days promotion)\n\n"
        confirmation_text += "Please confirm your advertisement:"
        
        # MODIFIED: Only show payment confirmation in real mode
        if self.payment_mode == "real":
            keyboard = [
                [InlineKeyboardButton("✅ Confirm & Pay", callback_data="confirm_purchase")],
    ...
    async def confirm_purchase(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Confirm and process purchase - ENHANCED with better confirmation"""
        query = update.callback_query
        await query.answer()
        ...
        async with aiosqlite.connect(DB_NAME) as db:
            ...
            # MODIFIED: Auto-complete payment in dummy mode with better confirmation
            if self.payment_mode == "dummy":
                status = "completed"
                purchase_id = f"PUR{datetime.now().strftime('%Y%m%d%H%M%S')}"
                message = f"🎉 *PURCHASE COMPLETED!* 🎉\n\n📄 *Purchase ID:* {purchase_id}\n\n"
            else:
                status = "pending" 
            ...
            await db.execute("""
                ...
                VALUES (?, ?, 'Advertisement', 50.00, ?, ?, ?)
            """, (user_id, query.from_user.username, status, datetime.now().isoformat(), json.dumps(ad_data)))
        ...
        # ENHANCED: Different messages based on payment mode with better details
        if self.payment_mode == "dummy":
            confirmation_details = (
                ...
                f"✨ Your ad will start running immediately!\n\n"
                f"📊 *Next Steps:*\n"
                f"• Ad will appear in rotation\n"
    ...
    async def start_vip_purchase(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await query.edit_message_text(
            ...
            "Select your VIP membership duration:\n\n"
            "💎 *VIP Benefits:*\n"
    ...
    async def handle_vip_duration(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        duration_map = {
            '1': {'months': 1, 'amount': 10.00, 'text': '1 Month'},
            '3': {'months': 3, 'amount': 25.00, 'text': '3 Months'},
            '6': {'months': 6, 'amount': 45.00, 'text': '6 Months'},
            '12': {'months': 12, 'amount': 80.00, 'text': '1 Year'}
        }
    ...
    async def confirm_vip_purchase(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        async with aiosqlite.connect(DB_NAME) as db:
            # MODIFIED: Auto-complete payment in dummy mode
            if self.payment_mode == "dummy":
                status = "completed"
                purchase_id = f"VIP{datetime.now().strftime('%Y%m%d%H%M%S')}"
                
                # Also add to VIP members table
                ...
                message = f"🎉 *VIP MEMBERSHIP ACTIVATED!* 🎉\n\n📄 *Membership ID:* {purchase_id}\n\n"
            else:
                status = "pending"
                message = "🎉 *VIP PURCHASE CONFIRMED!* 🎉\n\n"
            
            await db.execute("""
                INSERT INTO purchases (user_id, username, product_type, amount, status, created_at, data)
                VALUES (?, ?, 'VIP Membership', ?, ?, ?, ?)
            """, (user_id, query.from_user.username, vip_data['amount'], status, datetime.now().isoformat(), json.dumps(vip_data)))
        ...
        # ENHANCED: Better confirmation messages
        if self.payment_mode == "dummy":
            confirmation_text = (
                f"{message}"
                f"👑 *Welcome to VIP!* 👑\n\n"
                ...
                f"• Status: ✅ ACTIVE\n"
                f"• Expires: {(datetime.now() + timedelta(days=30 * vip_data['duration_months'])).strftime('%Y-%m-%d')}\n\n"
                f"✨ *Your VIP benefits are now active!*\n\n"
                f"🚀 *Next Steps:*\n"
                f"• Access VIP channels immediately\n"
                f"• Enjoy exclusive content\n"
                f"• Use priority support\n\n"
                f"📞 *Support:* {AUTOADV_BOT_USERNAME}"
            )
        else:
            confirmation_text = (
                f"{message}"
                f"👑 *VIP Purchase Confirmed!* 👑\n\n"
                f"💎 *Membership Details:*\n"
                f"• Duration: {vip_data['duration_text']}\n"
                f"• Amount: ${vip_data['amount']:.2f}\n"
                f"• Status: ⏳ PENDING PAYMENT\n\n"
                ...
                f"• Include memo if possible\n"
                f"• After payment, use /autoadvverify [TX_HASH]\n"
                f"• VIP access activates after payment confirmation\n\n"
                f"📞 *Support:* {AUTOADV_BOT_USERNAME}"
            )
    ...
    def setup_handlers(self):
        ...
        # Enhanced purchase conversation handler - FIXED with proper per_message=False
...
"""
                ...
                # Create keyboard with optional button
                keyboard = []
                if ad and ad[7] == 1 and ad[8] and ad[9]:  # has_button, button_text, button_url
                    keyboard.append([InlineKeyboardButton(ad[8], url=ad[9])])
    ...
    def setup_handlers(self):
        ...
        ad_purchase_conv = ConversationHandler(
            ...
            states={
                ...
                AD_IMAGE_OPTION: [CallbackQueryHandler(autoadv_bot.receive_image_option, pattern="^image_")],
                AD_IMAGE_UPLOAD: [MessageHandler(filters.PHOTO, autoadv_bot.receive_image_upload)],
                AD_BUTTON_OPTION: [CallbackQueryHandler(autoadv_bot.receive_button_option, pattern="^button_")],
                AD_BUTTON_DETAILS: [MessageHandler(filters.TEXT & ~filters.COMMAND, autoadv_bot.receive_button_details)],
        ...
        # VIP purchase handlers
        self.app.add_handler(CallbackQueryHandler(autoadv_bot.start_vip_purchase, pattern="^buy_vip$"))
        self.app.add_handler(CallbackQueryHandler(autoadv_bot.handle_vip_duration, pattern="^vip_"))
        self.app.add_handler(CallbackQueryHandler(autoadv_bot.confirm_vip_purchase, pattern="^confirm_vip$"))
